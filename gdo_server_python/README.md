...
- **Database Access**: The server only accesses the `app_gpt_gdo` database in MotherDuck, specifically the `prodotti_xeel_shop` table in the `main` schema. It performs read-only operations (SELECT queries only).
